...
def introduce():
    action()
    response = requests.get(f"http://project-app-svc:2346/todos")
    response_value = response.json() #[{"todo": "Hello"},{"todo": "World"}]
    todo_list = [item_dict['todo'] for item_dict in response_value] #["Hello", "World"]
    return render_template('introduction.html', todo_list=todo_list)

@app.route("/",methods=["POST"])
def create_todo():
    todo = request.form.get("createtodo")
    requests.post(f"http://project-app-svc:2346/todos", json = {'todo': todo})
